...
var createCellFromURIValuePair = function(data) {
	...
	if (data[SF_NAME_SPACE + 'type'] == 'Node') {
		...
		cell.attrs['.connection']['stroke-width'] = data[SF_NAME_SPACE
				+ 'stroke_width'];
...
$(function() {
	 ...
	 * var flgs = new Array();
	 *  // Property for (var i = 0; i < sfPropAry.length; i++) { var obj =
	 * sfPropAry[i]; var nodeId = obj.id; $.each(obj, function(k, v) { if (k !=
	 * "type") { subs.push(nodeURI + nodeId); pres.push(sfURI + k);
	 * objs.push(v); flgs.push("true"); }
	 *
	 * });
	 *
