...
public class MetadataController {
	...
	public @ResponseBody String addWorkflow(@RequestParam String projectID,
			@RequestParam String workflowJSON, @RequestParam String properties)
		...
		String query = "SELECT DISTINCT ?o WHERE {<" + SF.NS + "project#"
				+ projectID + "> <" + SF.NS + "child> ?o . }";
		...
		url = deleteUrl;
		postStr = "subject=" + SF.NS + "project#" + projectID + "&predicate="
				+ SF.NS + "child";
		post(url, postStr);
	...
	public @ResponseBody String addWorkflow2(@RequestParam String projectID,
			@RequestParam String workflowJSON, @RequestParam String properties)
			throws JsonProcessingException, IOException {
		 ...
		 *
		 *
		 * String url = sparqlUrl; String query =
		 * "SELECT DISTINCT ?o WHERE {<"+SF
		 * .NS+"project#"+projectID+"> <"+SF.NS+"child> ?o . }"; String postStr
		 * = "query="+query+"&infFlag=false"; JsonNode deleteJson =
		 * mapper.readTree(post(url,postStr));
		 *
		 * for (JsonNode target :deleteJson.get("results").get("bindings")){
		 * String targetURI = target.get("o").get("value").asText(); url
		 * =deleteUrl; postStr = "subject="+targetURI; post(url,postStr); }
		 *
		 * url = deleteUrl; postStr =
		 * "subject="+SF.NS+"project#"+projectID+"&predicate="+SF.NS+"child";
		 * post(url,postStr);
		 */
		...
		while (fieldNames.hasNext()) {
			...
			/*
			 * celldata.setProjectID(projectID); System.out.println(celldata);
		 ...
		 *
		 *
		 * try { URL urlObject = new URL(addUrl); HttpURLConnection connection =
		 * null;
		 *
		 * try { connection = (HttpURLConnection) urlObject.openConnection();
		 * connection.setDoOutput(true); connection.setRequestMethod("POST");
		 * OutputStream os = connection.getOutputStream(); String postStr =
		 * "subject=" + subject + "&predicate=" + predicate + "&object=" +
		 * object + "&literalFlag=" + flag; // System.out.println(postStr);
		 * PrintStream ps = new PrintStream(os); ps.print(postStr); ps.close();
		 *
		 * if (connection.getResponseCode() == HttpURLConnection.HTTP_OK) { try
		 * (InputStreamReader isr = new InputStreamReader(
		 * connection.getInputStream(), StandardCharsets.UTF_8); BufferedReader
		 * reader = new BufferedReader(isr)) { String line; while ((line =
		 * reader.readLine()) != null) { // logger.info(new //
		 * Throwable().getStackTrace()[0].getClassName() + // " : "+ new //
		 * Throwable().getStackTrace()[0].getMethodName() + // " : line >>> ");
		 * // System.out.println(line); } } } } finally { if (connection !=
		 * null) { connection.disconnect(); } } } catch (IOException e) {
		 * e.printStackTrace(); }
		 */
		...
		while (propChildren.hasNext()) {
			...
			/*
			 * DataObject propdata = mapper.readValue(prop,Property.class);
			 * propdata.setProjectID(projectID); propdata.add(addUrl);
			 */
		}
	...
	public @ResponseBody String getWorkflow(@RequestParam String projectID) {
		String result = "";
		String query = "PREFIX sf: <http://sfweb.is.k.u-tokyo.ac.jp/> SELECT ?childURI ?v ?o WHERE {?projectURI sf:child ?childURI . ?projectURI sf:projectId \""
				+ projectID + "\" . ?childURI ?v ?o. } ";
		String flag = "false";
		String url = sparqlUrl;
